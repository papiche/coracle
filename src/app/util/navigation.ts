/**
 * navigation.ts — Application-level router extensions and serializers.
 *
 * This module extends the base routing engine (src/util/router.ts) with
 * Nostr-specific decoders (npub, nprofile, nevent, naddr, …) and registers
 * named route segments used throughout the app (notes, people, relays, …).
 *
 * ⚠️  Do NOT import directly from this file in new code.
 *      Use the re-export barrel  src/app/util/index.ts  instead.
 *
 * Background: there are two router files in the project:
 *   • src/util/router.ts       — the generic routing engine class
 *   • src/app/util/navigation.ts — this file (Nostr-specific extensions)
 * Keeping them in separate namespaces avoids IDE auto-import confusion.
 */
...
// Router instance with Nostr-specific extensions
